...
def load_loopbacks(path='networkinfo.json'):
    """Carga los SIDs de la red (sin máscara)"""
    try:
...
def configurar_extremos_acceso(nodo_cliente, nodo_servidor, vlan_id, qos_classes):
    """ Crea dinámicamente IPs y marcado DSCP para N flujos infiriendo el tipo mediante la latencia """
    cmds_gnb = [
    ...
    cmds_upf = [
        ...
        f"ip link set up eth1.{vlan_id}",
        f"ip link set up eth2.{vlan_id}",
        f"ip -6 addr add fd00:{vlan_id}:d::1/64 dev eth1.{vlan_id} 2>/dev/null", # <--- D va en eth1 (Hacia hosts)
        f"ip -6 addr add fd00:{vlan_id}:c::1/64 dev eth2.{vlan_id} 2>/dev/null", # <--- C va en eth2 (Hacia ru)
    ...
    # Iterar sobre las clases inyectadas en la petición y asignar DSCP leyendo el delay
    idx = 2
    for cls_name, cls_data in qos_classes.items():
        req_delay = float(cls_data.get("delay", 100))
        
        # Mapeo Inteligente basado en IETF (5QI to DSCP Mapping)
        if req_delay <= 5:
            dscp_int = "0x30" # URLLC (Group 1 / TNA)
        elif req_delay <= 20:
            dscp_int = "0x26" # Video (Group 2 / TNB)
        elif req_delay <= 50:
            dscp_int = "0x12" # Telemetría (Group 3 / TNC)
        else:
            dscp_int = "0x00" # Best Effort (Group 4 / TND)
        ...
        cmds_upf.extend([
            f"ip -6 addr add fd00:{vlan_id}:d::{idx}/64 dev eth1.{vlan_id} 2>/dev/null", # <--- CORREGIDO a eth1
...
def configurar_routers_borde(vlan_id, req_cir_total, qos_classes, ruta_ida_sids, ruta_vuelta_sids, req_delay_min):
    ...
    for cls_name, cls_data in qos_classes.items():
        ...
        # Inyectar reglas HTB (Granularidad Fina) leyendo DSCP Interno
        cmds_rg.extend([
        ...
        if dscp_ext == "0x04":
            ...
            # Filtros para el Router de Acceso (RG)
            cmds_rg.extend([
            ...
            # Filtros para el Router de Salida (RU)
            cmds_ru.extend([
...
def inyectar_comandos_router(slice_id, req_cir_total, ruta_asignada, req_delay_min, qos_classes):
    ...
    print(f"[SRC] Re-verificando estado crítico de los nodos de la ruta: {ruta_asignada}")
    if not check_containers_running(ruta_asignada):
        ...
        return False
    print("[SRC] ✅ Nodos de la ruta verificados. Procediendo a inyectar comandos...")
    ...
    # Generar ruta vuelta asimétrica
